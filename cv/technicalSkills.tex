%-----------TECHNICAL SKILLS-----------
\section{Technical Skills}
 \begin{itemize}[leftmargin=0.0in, label={}]
    \small{\item{
     \textbf{\normalsize{Languages :}}{\normalsize{Python, TypeScript, C++, SQL}} \\
     \vspace{4pt}
     \textbf{\normalsize{Relevant courses :}}{\normalsize{Machine Learning, Data Analytics, Natural Language Processing, Data Structures \& Algorithms, Databases and Information Systems, Statistical Signal Processing, Language Technology, Computer Vision, Neural Networks}} \\
     \vspace{4pt}
     \textbf{\normalsize{Technologies :}}{\normalsize{PyTorch, PyTorch Lightning, Hydra, Scikit-Learn, Pandas, PostgreSQL, Langchain, Docker, Tableau, Seaborn, FastAPI, Livekit, Supabase, Redis, React, NodeJs, Github Actions, Google Cloud, Git, Tailwind CSS, MS Office}}
    }}
    
 \end{itemize}